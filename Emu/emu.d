...
//
// Since there's no real mapping between token lines and their value in this map,
// it must be checked if contained here before being switched on to
// add those bytes (for example, CB here) and then the suffix from
// sufxMap in order to properly represent the token line
private const ubyte[string] cbMap;
private const ubyte[string] ddMap;
private const ubyte[string] ddcbMap;
...
public template z80()
{
    public ubyte[] assemble(string tdata)
    {
        ...
        foreach (i; 0..tokens.length)
        {
            ...
            else if (line in cbMap)
            {
                bytes ~= [ 0xcb ];
                bytes ~= cbMap[line];
                bytes ~= literals[i];
            }
            else if (line in ddMap)
            {
                bytes ~= [ 0xdd ];
                bytes ~= ddMap[line];
                bytes ~= literals[i];
            }
            else if (line in ddcbMap)
            {
                bytes ~= [ 0xdd, 0xcb ];
                bytes ~= literals[i];
                bytes ~= ddcbMap[line];
            }
            else if (line in edMap)
            {
                bytes ~= [ 0xed ];
                bytes ~= ddcbMap[line];
                bytes ~= literals[i];
            }
            else if (line in fdMap)
            {
                bytes ~= [ 0xfd ];
                bytes ~= ddcbMap[line];
                bytes ~= literals[i];
            }
            else if (line in fdcbMap)
            {
                bytes ~= [ 0xfd, 0xcb ];
                bytes ~= literals[i];
                bytes ~= ddcbMap[line];
...
// This is used for converting a token list of x86 to Z80 as bytes or a token list
//...